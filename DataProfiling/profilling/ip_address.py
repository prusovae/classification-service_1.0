...
def tokenize_ip_address(value=None):
    return [match_value for match_value in re.findall(
        ...
        '(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.'
        '(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[0-9])', value.lower())] if value else []
...
def ip_address(param_dict=None):
    ...
    return {'dmn': 'DMN_IP_ADDRESS', 'percent': percent_diff_ip_address(percentage, mdata_match_percent if md_pattern_ip_address(
        field_name) else 0)} if percentage > min_conformance_percent else {'dmn': 'DMN_IP_ADDRESS', 'percent': 0.0}
