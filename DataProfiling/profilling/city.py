...
city_pattern = '(?!.*(type|kind|date|period|owner|public|capac).*).*(city|gorod|town|locality|settle).*'


def city_md_pattern(field_name):
    return True if field_name and re.findall(city_pattern, field_name) else False
...
def city(param_dict=None):
    ...
    return {'dmn': 'DMN_CITY', 'percent': percent_diff_acct_number(percentage, mdata_match_percent)} if percentage > min_conformance_percent else {'dmn': 'DMN_CITY', 'percent': 0.0}
