...
email_value_pattern = '\A[a-z0-9!#$%&*+/=?^_‘{|}~-]+(?:\.[a-z0-9!#$%&*+/=?^_' \
...
def is_email_value(value):
    return True if value and [value for value in value.split(' ')
                              if re.findall(email_value_pattern, value)] \
        else False
...
def email(param_dict=None):
    ...
    return {'dmn': 'DMN_EMAIL', 'percent':
        percent_diff_email(percentage, mdata_match_percent)} \
        if percentage > min_conformance_percent \
        else {'dmn': 'DMN_EMAIL', 'percent': 0.0}
