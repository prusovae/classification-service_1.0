...
org_name_pattern = '(?!.*(day|width|num|form|kind|flag|flgdat|id|vers|type' \
                   '|amt|amount|stat|key|sum|date|time|count|cost|period).*)' \
                   '.*(corp_nam|fssp_nam|custn|where|customer|benef|firm' \
                   '|legal|who|issue|org|comp|party|employ|emitent|depon|' \
                   'emp_nam|agency|creditor|institut|payer|issuer|bank' \
                   '|authority|investor).*'


def org_name_md_pattern(field_name):
    return True if field_name and re.findall(org_name_pattern, field_name) \
        else False

def is_org_name_value(value):
    return True if value and [val for val in value.split()
                              if val.upper() in org_name_prefix] else False
...
def org_name(param_dict=None):
    ...
    return {'dmn': 'DMN_COMPANY'
            , 'percent': percent_diff_org_name(percentage, mdata_match_percent)} \
        if percentage > min_conformance_percent \
        else {'dmn': 'DMN_COMPANY', 'percent': 0.0}
