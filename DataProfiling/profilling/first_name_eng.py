...
first_names_eng_pattern = '(?!.*(flag|flg|period|rate|currency|pk|date' \
                          '|time|struct|state).*).*' \
                          '(f_name|fname|fst|first|given).*'


def first_name_eng_md_pattern(field_name):
    return True \
        if field_name and re.findall(first_names_eng_pattern, field_name) \
        else False
...
def is_first_name_eng_value(value):
    ...
    return True \
        if value and [value for value in value if value in first_names_eng] \
        else False
...
def first_name_eng(param_dict=None):
    ...
    return {'dmn': 'DMN_FIRST_NAME', 'percent':
        percent_diff_first_name_eng(percentage, mdata_match_percent)} \
