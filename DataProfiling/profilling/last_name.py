import re
from profilling.dict_loader import last_names
import logging
logger = logging.getLogger(__name__)
logger.info("модуль запустился")
last_names_pattern = (
            '(?!.*(exposure|insurance|uid|token|pk|full|date|time|logical|vers|state|status|collection|tpl_n|ts|'
            + 'process|amount|period|summ|count|run|dll_n|tooln|measur|start|end).*).*(l_name|lname|last|sur|family).*')


def last_name_md_pattern(field_name):
    return True if field_name and re.findall(last_names_pattern, field_name) else False

def is_last_name_value(value):
    if not value:
        return False
    value = value.replace(',', '')
    value = value.replace('.', '')
    value = value.lower().split()
    return True if value and [value for value in value if value in last_names] else False


def percent_diff_last_name(percent, diff):
    return 100 if percent + diff > 100 else round(percent + diff, 1)

def last_name(param_dict=None):
    """
    Тут будет docstring
    """
    count_match = 0
    ...
    if last_name_md_pattern(field_name):
